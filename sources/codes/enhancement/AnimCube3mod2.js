...
function AnimCube3(params) {
  ...
  function acjs_save_cube() {
    var config = {};
    config.eye = [];
    config.eyeX = [];
    config.cube = [];
    vCopy(config.eye, eye);
    vCopy(config.eyeX, eyeX);
    for (var i=0; i < 6; i++) {
      config.cube[i] = [];
      for (var j=0; j < 9; j++)
        config.cube[i][j] = cube[i][j];
    }
    return(config);
  }

  function acjs_restore_cube(config) {
    vCopy(eye, config.eye);
    vCopy(eyeX, config.eyeX);
    vNorm(vMul(eyeY, eye, eyeX));
    for (var i=0; i < 6; i++)
      for (var j=0; j < 9; j++)
        cube[i][j] = config.cube[i][j];
    paint();
  }

  save_cube = acjs_save_cube;
  restore_cube = acjs_restore_cube;
}
