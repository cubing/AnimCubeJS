/*
  Note:
  This version has been modified to handle long sequences without the
  animation slowing down as they get progressively longer.
  For the sake of expediency, some tradeoffs were made which are that
  Info Text and Metric are disabled.
*/
/**
...
function AnimCube2(params) {
  ...
  function init() {
    ...
    if (scramble == 0) {
      ...
      if (param != null) {
        if ("random" == (param))
          param = randMoves(2, randMoveCount);
        demoMove = param == ("#") ? move : getMove(param, false);
  ...
  function moveTextFunc(move, start, end, f) {
    if (start >= move.length)
      return "";
    var w = graphics.measureText('I').width;
    var n = Math.floor(width/w);
    var s = "";
    if (end - start > n) {
      if (f)
        start = end - n;
  ...
  */
  ...
  function paint() {
    ...
    if (!demo && move.length > 0) {
	  ...
	  var progress = (width - 2) * movePos / move[curMove].length;
	  ...
	  var s = movePos + " / " + move[curMove].length;
  ...
  function drawMoveTextFunc(g, y) {
    ...
    var s = move[curMove];
    var s1 = moveTextFunc(s, 0, pos, 1);
    var s2 = turnTextFunc(s, pos);
    var s3 = moveTextFunc(s, pos + 1, s.length, 0);
    if (moveTextSpace) {
      if (s2 == '')
